...
2. **[新規]** を選び、 **[基本]** タブで、以下の詳細を入力します。
   ...
   | **設定**                   | **推奨値**             | **説明**                                    |
   | ------------------------ | ------------------- | ----------------------------------------- |
   | Azure Data Explorer プール名 | [yourname]contosoadx | これは、Azure Data Explorer プールに付けられる名前です。    |
   | ワークロード                   | Compute optimized   | このワークロードにより、CPU から SSD へのストレージの比率が高くなります。 |
   | ノード サイズ                  | Small (4 コア)        | この演習のコストを削減するために、最小サイズに設定します。             |
   
   > **Note**
   > 
   > Azure Data Explorer プールで使用できる名前には、特定の制限があることに注意してください。 この名前には、4 文字から 24 文字の間で小文字と数字のみを含める必要があります。また、先頭は文字で始める必要があります。
...
2. **+** (新しいリソースの追加) > **[Data Explorer データベース]** を選び、次の情報を貼り付けます。
   ...
   | **設定**     | **推奨値**             | **説明**                                                               |
   | ---------- | ------------------- | -------------------------------------------------------------------- |
   | プール名       | [yourname]contosoadx | 使用する Data Explorer プールの名前                                            |
...
3. **[接続先]** メニューで、[*[yourname]contosoadx*] を選択します。
...
5. 次のコマンドを貼り付け、**[実行]** を選択して、StormEvents テーブルを作成します。
   ...
   > **Note**
   > 
   > テーブルが正常に作成されたことを確認します。 左側のペインで、**[Data]** を選び、*[yourname]contosoadx* のその他のメニューを選び、**[最新の情報に更新]** を選択します。 *[yourname]contosoadx* で **[テーブル]** を展開し、*StormEvents* テーブルが一覧に表示されていることを確認します。
